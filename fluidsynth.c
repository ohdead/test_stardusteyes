...
#if 0
static char *win32_ansi_to_utf8(const char *ansi_null_terminated_string); // K_UPDATE
#endif
...
int process_o_cmd_line_option(fluid_settings_t *settings, char *optarg)
{
    ...
    switch (fluid_settings_get_type(settings, optarg))
    {
        ...
        case FLUID_STR_TYPE: {
/* char *u8_val = val; */ // K_UPDATE
#if 0
u8_val = win32_ansi_to_utf8(val); // K_UPDATE
#endif
            //            if (fluid_settings_setstr(settings, optarg, u8_val) != FLUID_OK)
            if (fluid_settings_setstr(settings, optarg, val) != FLUID_OK) // K_UPDATE
            {
                fprintf(stderr, "Failed to set string parameter '%s'\n", optarg);
#if 0
free(u8_val); // K_UPDATE
#endif
                return FLUID_FAILED;
            }
#if 0
...
int main(int argc, char **argv)
{
    ...
    // update argc and argv: utf-16 command line string to utf-8 arguments
    {
        ...
        // parses a unicode command line string and get an array of pointers to the command line
        // arguments. allocates a new argv array.
...
/* reading / setting options from the command line */
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        if (optarg && optarg[1] == ':') /* colon follows if switch argument expected */
        {
            ...
            else
            {
                ...
                //                if(optarg[0] == '-')
                if ((optarg[0] == '-') && ((optarg[1] != '\0') || (c != 'F'))) // K_UPDATE
                {
    ...
    for (i = arg1; i < argc; i++)
    {
        /* const char *u8_path = argv[i]; */ // K_UPDATE

#if 0
// #if defined(_WIN32) // K_UPDATE
...
u8_path = u8_buf;
        ...
        if (fluid_is_midifile(argv[i])) // K_UPDATE
        {
            continue;
        }

        //        if (fluid_is_soundfont(u8_path))
        if (fluid_is_soundfont(argv[i])) // K_UPDATE
        {
            //           if (fluid_synth_sfload(synth, u8_path, 1) == -1)
            if (fluid_synth_sfload(synth, argv[i], 1) == -1) // K_UPDATE
            {
